...
class ProxyConnector():
    ...
    def __init__(self,proxyURLs,appURL):
        ...
        url = urlparse(appURL)
        logger.debug("[ProxyConnector]: Connecting to AppServer in " + url.netloc)       

        conn = httplib.HTTPConnection(url.netloc)
        conn.request('POST','/socket.io/1/')
        resp = conn.getresponse()
        hskey = resp.read().split(':')[0]
        logger.debug("[ProxyConnector]: hskey is " + hskey)
        self.appWS = websocket.WebSocketApp(appURL+ "/socket.io/1/websocket/" + hskey,
    ...
    def App_on_message(self,ws, message):
        logger.debug("[ProxyConnector]: received message " + message)
...
if __name__ == "__main__":
    if (len(sys.argv) < 2):
        print "Usage: python ProxyIOSocketConnector.py <proxyurl1,proxyurl2,proxyurl3> <appurl>"
        exit()
    if (len(sys.argv) == 2):
        proxylist = []
    else:
